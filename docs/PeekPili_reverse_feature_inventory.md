...
- `PENDING (migration)` current project still contains implementation gaps:
  - Android `sourceRuntimeProbe/sourceRuntimeExecute` now includes PHP bridge-aligned runtime env and working-directory defaults, plus GoProxy runtime start/status/stop generic action path; it still remains a generic command bridge (not full reverse plugin lifecycle parity for all engines).
  - Android GoProxy bridge now has start/stop/status/command-prepare, runtime-state diagnostics (including last-exit snapshot), effective arg normalization baseline (`-port`/`-danmu-dir`), foreground-service notification baseline, wake/wifi lock baseline, notification stop action pid-termination attempt, and runtime auto-reconciliation for process/service state drift; remaining gap is full reverse-equivalent behavior validation under real proxy workloads (signal strategy robustness and long-run stability).
