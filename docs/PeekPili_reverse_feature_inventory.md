...
- `PENDING (migration)` current project still contains implementation gaps:
  ...
  - Android GoProxy bridge now has start/stop/status/command-prepare, runtime-state diagnostics, effective arg normalization baseline (`-port`/`-danmu-dir`), and foreground-service notification baseline; remaining gap is full reverse-equivalent service behavior (wake/wifi lock strategy, action routing, long-run workload validation).
