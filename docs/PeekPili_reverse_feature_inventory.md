...
- `PENDING (migration)` current project still contains implementation gaps:
  ...
  - Android `loadJar` + Jar lifecycle + spider business method surface (`home/search/detail/player/action/setRecent`) now have baseline reflective bridge support with per-spider context reuse, one-time init/context hook attempts (`init/initialize/setContext`), destroy/release lifecycle callback attempts, runtime-state diagnostics, and auto crash-state sync on invoke success/failure; however it still lacks reverse package `JarLoader`-equivalent guarantees for all plugin side-effects and full compatibility validation across real source jars.
