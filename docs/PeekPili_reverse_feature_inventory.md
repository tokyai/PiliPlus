...
- `PENDING (migration)` current project still contains implementation gaps:
  ...
  - Android GoProxy bridge now has start/stop/status/command-prepare and runtime-state diagnostics baseline, but still lacks reverse package process argument/notification/service-side behavior equivalence validation under real proxy workloads.
