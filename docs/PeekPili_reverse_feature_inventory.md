...
- `PENDING (migration)` current project still contains implementation gaps:
  ...
  - Android `loadJar` + Jar lifecycle + spider business method surface (`home/search/detail/player/action/setRecent`) now have baseline reflective bridge support with per-spider context reuse, destroy/release lifecycle callback attempts, and runtime-state diagnostics, but still lack reverse package `JarLoader`-equivalent guarantees for all plugin side-effects and full compatibility validation across real source jars.
