...
- `PENDING (migration)` current project still contains implementation gaps:
  ...
  - Android GoProxy bridge now has start/stop/status/command-prepare, runtime-state diagnostics, and effective arg normalization baseline (`-port`/`-danmu-dir`), but still lacks reverse package foreground notification/service-side behavior equivalence validation under real proxy workloads.
